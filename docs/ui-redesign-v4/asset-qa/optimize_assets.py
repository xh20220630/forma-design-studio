...
def optimize():
    ...
    for name in SCENES:
        ...
        img = original.copy()
        img.thumbnail(
            (1920, 640) if name.startswith('hero') else (1200, 750), Image.Resampling.LANCZOS
        )
        ...
        if name.startswith('hero'):
            wide = original.crop((0, 100, original.width, 650)).resize(
                (1920, 486), Image.Resampling.LANCZOS
            )
            ...
            records.append(
                {
                    ...
                    'mode': wide.mode,
                    'source_size': list(original.size),
                    'crop': [0, 100, original.width, 650],
                }
            )
            compact = original.crop((1350, 0, 2074, 724)).resize(
            ...
            records.append(
                {
                    'file': target.name,
                    'size': list(compact.size),
                    'bytes': target.stat().st_size,
                    'mode': compact.mode,
                    'source_size': list(original.size),
                    'crop': [1350, 0, 2074, 724],
                }
            )
    for name in [
        'rin-full-body',
        ...
        height = (
            128 if name == 'rin-avatar' else 640 if name in ('rin-full-body', 'rin-theme') else 320
        )
        img = original.copy()
        ...
        safe.paste(
            content, ((safe.width - content.width) // 2, (safe.height - content.height) // 2)
        )
        img = safe
