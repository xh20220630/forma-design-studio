...
sheet = Image.new(
    'RGB',
    (columns * tile_width, ((len(records) + columns - 1) // columns) * tile_height),
    '#F5F6F8',
)
    ...
    if img.mode == 'RGBA':
        ...
        for cy in range(0, 278, 16):
            for cx in range(0, 376, 16):
                draw.rectangle(
                    (cx, cy, cx + 15, cy + 15),
                    fill='#E4E9EF' if (cx // 16 + cy // 16) % 2 else '#F6F8FA',
                )
    thumb = img.copy()
    thumb.thumbnail((376, 270), Image.Resampling.LANCZOS)
    tile.paste(
        thumb,
        ((376 - thumb.width) // 2, (278 - thumb.height) // 2),
        thumb if thumb.mode == 'RGBA' else None,
    )
    sheet.paste(tile, (x + 12, y + 12))
    ImageDraw.Draw(sheet).text(
        (x + 14, y + 296), f'{record["file"]}  {img.width}x{img.height}', fill='#191E28'
    )
    checks.append(
        {
            'file': record['file'],
...
summary = {
    ...
    'production_bytes': sum(record['bytes'] for record in records),
    'original_rin_opaque_rgb_exact': True,
    'checks': checks,
}
