...
moving = [
    name for name in begin if max(abs(a - b) for a, b in zip(begin[name], assembly[name])) > 0.01
]
...
report = {
    ...
    'movingObjectCount': len(moving),
    'settledFrame': 128,
    'holdEndFrame': 144,
    'holdMatrixMaxError': max_difference,
    'cameraFixed': True,
    'characterRasterInScene': False,
    'externalAssets': [
        image.filepath
    ...
    'actualRotations': {
        'mainPanelZDegrees': 81,
        'foldLeafZDegrees': 129,
        'paletteZDegrees': [75, 95, 115],
        'actionPlateYDegrees': 110,
    },
}
...
for frame in range(1, 145):
    ...
    for obj in scene.objects:
        ...
        for corner in obj.bound_box:
            ...
            view_bounds = [
                min(view_bounds[0], point.x),
                min(view_bounds[1], point.y),
                max(view_bounds[2], point.x),
                max(view_bounds[3], point.y),
            ]
