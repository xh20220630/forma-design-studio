...
export const seedTemplates: DesignTemplate[] = [
  {
    id: 'moss',
    name: 'Moss',
    description: '自然的色彩，安静的日常。为有温度的生活方式品牌而生。',
    category: '电商',
    author: 'Forma Studio',
    cover: 'commerce',
    featured: true,
  ...
  },
  {
    id: 'midnight',
    name: 'Midnight',
    description: '深邃底色与鲜明数据，兼具专业感和未来感。',
    category: '仪表盘',
    author: 'Forma Studio',
    cover: 'finance',
    featured: true,
  ...
  },
  {
    id: 'paper',
    name: 'Paper',
    description: '编辑式排版与细腻留白，让内容成为主角。',
    category: '品牌网站',
    author: 'Forma Studio',
    cover: 'commerce',
    tokens: {
  ...
  },
  {
    id: 'tide',
    name: 'Tide',
    description: '海岸色调与轻盈的布局，为每次探索留出空间。',
    category: '品牌网站',
    author: 'Forma Studio',
    cover: 'travel',
    featured: true,
  ...
  },
  {
    id: 'studio',
    name: 'Studio',
    description: '清晰、克制、高效。适合从零构建现代工作台。',
    category: '仪表盘',
    author: 'Forma Studio',
    cover: 'dashboard',
    tokens: { ...baseTokens },
  },
  {
    id: 'iris',
    name: 'Iris',
    description: '柔和的紫色与亲切的界面，让工作也充满灵感。',
    category: 'SaaS',
    author: 'Forma Studio',
    cover: 'dashboard',
    tokens: {
      ...
      nodes: [
        {
          ...
          height: 44,
          fill: tokens.primary,
          color: tokens.background === '#171b20' ? '#171b20' : '#ffffff',
          text: '开始使用',
          fontSize: 14,
...
/**
 * 根据示例类型和主题构造页面，使种子项目有完整可编辑的初始内容。
 *
 * @param cover - 项目卡片使用的封面类型。
 * @param tokens - 设计主题或语义 Token 集合。
 * @param title - 界面显示的标题。
 * @returns 构造后的设计页面。
 */
function buildPage(cover: Project['cover'], tokens: ThemeTokens, title: string): DesignPage {
  ...
  const frame = (
    ...
    width: number,
    height: number,
    fill: keyof ThemeTokens = 'surface',
    radius = tokens.radius,
  ) =>
    add({
      name,
  ...
  if (cover === 'dashboard' || cover === 'finance') {
    ...
    text(
      'Eyebrow',
      finance ? 'YOUR MONEY, AT A GLANCE' : 'WORKSPACE / OVERVIEW',
      246,
      33,
      500,
      10,
    ...
    text(
      'Page subtitle',
      finance ? '欢迎回来，这是你的资产概况。' : '早上好，Alex。今天也是充满可能的一天。',
      246,
      133,
      650,
      14,
...
export function createProject(name: string, template?: DesignTemplate, description = ''): Project {
  ...
  return {
    ...
    description: description || '从一个想法，走向真实的产品。',
    category: selected.category,
    status: 'draft',
    themeId: selected.id,
    tokens,
    pages: [buildPage(template ? selected.cover : 'blank', tokens, name)],
    components: components(tokens),
    updatedAt: new Date().toISOString(),
    revision: 1,
    cover: template ? selected.cover : 'blank',
  };
}
