...
import type { AgentActionResult, AgentMessage, AgentSession, AgentTurnResponse, AgentActionType } from '@forma/schema/agent';
...
import { dataRoot, findProject, getState, mutateProject, readJson, transact, writeJson } from './store.ts';
import { validateId, validateProject, validateTokens, requireApproved, requireCurrentImage, tokenKeys } from './validate.ts';
import { activeProjectTokens, currentGeneration, designContext, designContextHash } from './design-context.ts';
...
const titles: Record<AgentActionType, string> = { create_project: '创建项目', update_tokens: '更新主题 Token', create_variables: '创建变量集合', create_component: '创建组件', generate_image: '生成设计图', reconstruct_design: '还原可编辑设计', preview_sync: '预览代码同步', approve_image: '确认设计图', apply_sync: '应用代码同步' };
...
const nonempty = (value: unknown, max = 20000): value is string => typeof value === 'string' && value.trim().length > 0 && value.length <= max;

async function sessionState() { return readJson<{ sessions: StoredAgentSession[] }>(sessionFile, { sessions: [] }); }
function publicSession(session: StoredAgentSession): AgentSession {
  const { pendingReviews, ...visible } = session;
  return visible;
}
async function changeSession(id: string, update: (session: StoredAgentSession) => StoredAgentSession) {
...
export async function getAgentSession(id: string) { return publicSession(await readSession(id)); }
export async function createAgentSession(input: Record<string, unknown>) {
  ...
  const session: StoredAgentSession = { id: randomUUID(), title: input.title?.trim() || '新对话', scope: input.projectId ? 'project' : 'global', ...(input.projectId ? { projectId: input.projectId } : {}), revision: 0, createdAt: now(), updatedAt: now(), messages: [], pendingReviews: [] };
...
async function planTurn(session: StoredAgentSession, project: Project | undefined, content: string): Promise<AgentPlan> {
  ...
  for (const action of result.actions) requireValue(record(action) && typeof action.type === 'string' && modelActions.has(action.type), '模型请求了不受支持的操作；图片确认与同步应用必须使用人工确认控件。', 502);
...
async function executeAction(action: AgentAction, initialProject: Project | undefined, session: StoredAgentSession, review = false) {
  let project: Project;
  const result: AgentActionResult = { id: randomUUID(), type: action.type, title: titles[action.type], status: 'completed', summary: '' };
  if (action.type === 'create_project') {
    ...
    if (action.type === 'update_tokens') {
      ...
      project = await saveMutation(project, current => ({ ...current, variableCollections: [...(current.variableCollections || []), collection], activeVariableModes: { ...current.activeVariableModes, [String(collection.id)]: Array.isArray(modes) ? modes[0] : undefined } }));
      ...
      project = await mutateProject(project.id, current => { assertRevision(current, project.revision); requireApproved(current); const next = validateProject({ ...current, ...design, status: 'in-progress' }); return { ...next, generation: { ...current.generation, contextHash: designContextHash(next) } }; });
...
export async function sendAgentMessage(id: string, input: Record<string, unknown>): Promise<{ status: number; body: AgentTurnResponse }> {
  ...
  requireValue(review ? record(action) && typeof action.type === 'string' && ['approve_image', 'reconstruct_design', 'apply_sync'].includes(action.type) : nonempty(contentInput), '请输入需求或选择有效的确认操作。');
  ...
  if (review) {
    requireValue(project && reviewAction!.projectId === project.id, '确认操作不属于当前会话绑定的项目。', 409);
    assertRevision(project, reviewAction!.revision);
  }
  requireValue(!runningSessions.has(id), '此会话正在执行，请等待当前回复完成。', 409);
  runningSessions.add(id);
  const reviewTitles: Partial<Record<AgentActionType, string>> = { approve_image: '确认当前设计图', reconstruct_design: '还原已确认的设计图', apply_sync: '确认应用已审查的代码同步' };
  const content = reviewAction ? reviewTitles[reviewAction.type]! : (contentInput as string).trim();
  const userMessage: AgentMessage = { id: randomUUID(), role: 'user', content, createdAt: now(), status: 'completed' };
  const assistantMessage: AgentMessage & { actions: AgentActionResult[] } = { id: randomUUID(), role: 'assistant', content: '正在处理…', createdAt: now(), status: 'pending', actions: [] };
  let status = 200;
  ...
  try {
    ...
    const plan = reviewAction ? { message: '', actions: [reviewAction] } : await planTurn({ ...session, messages: [...session.messages, userMessage] }, project, content);
    ...
    status = errorStatus(error) || 500; errorText = error instanceof ApiError ? error.message : '执行失败，请检查服务日志后重试。';
