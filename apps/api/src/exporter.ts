...
export function generateFiles(project: Project): ExportFile[] {
  ...
  const portableSurfaces = <T extends DesignPage | DesignComponent>(surfaces: T[]) => surfaces.map(surface => ({ ...surface, nodes: surface.nodes.map(node => {
...
async function safeGeneratedRoot(workspacePath: unknown) {
  ...
  } catch (error) { if (errorProperty(error, 'code') !== 'ENOENT') throw error; }
...
async function existingFile(root: string, filename: string) {
  ...
  } catch (error) { if (errorProperty(error, 'code') === 'ENOENT') return null; throw error; }
}

export async function previewSync(project: Project, { includeBaselines = false }: { includeBaselines?: boolean } = {}): Promise<SyncPreview> {
  ...
  for (const file of generateFiles(project)) {
    ...
    let status: SyncPreview['files'][number]['status'] = before === null ? 'added' : before === file.content ? 'unchanged' : 'modified';
...
export async function applySync(project: Project) {
  ...
  for (const file of preview.files.filter(file => file.status !== 'unchanged')) {
    ...
    } finally { await unlink(temporary).catch(error => { if (errorProperty(error, 'code') !== 'ENOENT') throw error; }); }
  }
