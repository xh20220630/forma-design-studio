...
import { generateDesign, generateImage, generateTheme, getProviderSettings, saveProviderSettings } from './provider.ts';
import { saveProvider, deleteProvider, getPrivateProvider, validateProvider, saveModelBindings } from './provider-settings.ts';
import { listProviderModels } from './provider-transport.ts';
import { bindWorkspace } from './workspaces.ts';
import { createAgentSession, getAgentSession, listAgentSessions, sendAgentMessage } from './agent.ts';
...
export function createApp() {
  ...
  app.get('/api/agent/sessions', async (req: ApiRequest, res) => res.json({ sessions: await listAgentSessions(req.query.projectId) }));
  app.post('/api/agent/sessions', async (req: ApiRequest, res) => res.status(201).json(await createAgentSession(req.body)));
  app.get('/api/agent/sessions/:id', async (req: ApiRequest, res) => res.json(await getAgentSession(req.params.id)));
  app.post('/api/agent/sessions/:id/messages', async (req: ApiRequest, res) => { const result = await sendAgentMessage(req.params.id, req.body); res.status(result.status).json(result.body); });
  app.get('/api/projects/:id', async (req: ApiRequest, res) => res.json(await findProject(validateId(req.params.id))));
  app.put('/api/projects/:id', async (req: ApiRequest, res) => {
  ...
  app.delete('/api/projects/:id', async (req: ApiRequest, res) => { await removeProject(validateId(req.params.id)); res.json({ ok: true }); });
  app.get('/api/settings', async (_req, res) => res.json(await getProviderSettings()));
  app.post('/api/settings', async (req: ApiRequest, res) => res.json(await saveProviderSettings(req.body)));
  app.post('/api/providers', async (req: ApiRequest, res) => res.status(201).json(await saveProvider(req.body)));
  app.put('/api/providers/:id', async (req: ApiRequest, res) => res.json(await saveProvider(req.body, req.params.id)));
  app.delete('/api/providers/:id', async (req: ApiRequest, res) => res.json(await deleteProvider(req.params.id)));
  app.post('/api/settings/models', async (req: ApiRequest, res) => res.json(await saveModelBindings(req.body)));
  app.get('/api/providers/:id/models', async (req: ApiRequest, res) => res.json(await listProviderModels(await getPrivateProvider(req.params.id))));
  app.post('/api/providers/probe', async (req: ApiRequest, res) => {
    const current = typeof req.body.id === 'string' ? await getPrivateProvider(req.body.id) : undefined;
  ...
  app.post('/api/generate/image', async (req: ApiRequest, res) => {
  ...
  app.post('/api/generate/approve', async (req: ApiRequest, res) => {
  ...
  app.post('/api/generate/design', async (req: ApiRequest, res) => {
  ...
  app.post('/api/generate/theme', async (req: ApiRequest, res) => res.json(await generateTheme(req.body.prompt)));
  app.post('/api/workspace/bind', async (req: ApiRequest, res) => {
    const snapshot = await findProject(validateId(req.body.projectId));
  ...
  app.post('/api/sync/preview', async (req: ApiRequest, res) => res.json(await previewSync(await findProject(validateId(req.body.projectId)))));
  app.post('/api/sync/apply', async (req: ApiRequest, res) => res.json(await synchronize(validateId(req.body.projectId), { expectedRevision: req.body.revision })));
  app.get('/api/projects/:id/export', async (req: ApiRequest, res) => {
  ...
  const handleError: ErrorRequestHandler = (error: unknown, _req, res, _next) => {
    const status = errorStatus(error) || (errorProperty(error, 'type') === 'entity.parse.failed' ? 400 : 500);
