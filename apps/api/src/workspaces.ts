...
export async function bindWorkspace(project: Project, input: Record<string, unknown>): Promise<WorkspaceBinding> {
  ...
  const match = /^https:\/\/github\.com\/([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+?)(?:\.git)?\/?$/.exec(typeof input.repo === 'string' ? input.repo : '');
  ...
  } catch (error) { throw new ApiError(400, `GitHub 克隆失败，请确认 Git 已安装且仓库、分支可访问。${String(errorProperty(error, 'stderr') || errorMessage(error)).slice(0, 500)}`); }
