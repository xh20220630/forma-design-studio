...
const configure = () =>
  api('/settings', 'POST', {
    apiKey: 'agent-test-secret',
    baseUrl: `http://127.0.0.1:${provider.address().port}/v1`,
    textModel: 'test-planner',
    imageModel: 'test-image',
  });
/**
 * 构造模拟模型的动作计划，明确当前场景预期执行的动作。
 *
 * @param actions - 待执行动作或已执行动作的结果集合。
 * @returns 模拟计划数据。
 */
...
test('provider errors and real conversation history persist across a new process', async () => {
  ...
  assert.match(reply.body.error, /API Key/);
...
test('global chat creates a bound project, tokens, variables and reusable components', async () => {
  ...
  plans.push(
    plan([
      { type: 'create_project', name: 'Agent app' },
      { type: 'update_tokens', tokens: { primary: '#334455' } },
      {
          ...
          variables: [
            {
              ...
              name: 'Accent',
              type: 'color',
              values: { light: '#334455', dark: '#ffffff' },
            },
          ],
        },
      },
          ...
          nodes: [
            {
              ...
              variableBindings: { color: { collectionId: 'semantic', variableId: 'accent' } },
            },
          ],
        },
      },
  ...
  const followup = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '过时消息',
    sessionRevision: 0,
  });
...
test('project chat isolates scope and keeps active theme modes and bindings consistent', async () => {
  ...
  const reply = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '修改主色',
    projectRevision: first.revision,
  });
  assert.equal(reply.status, 200);
  assert.equal(reply.body.project.themeModes.dark.primary, '#abcdef');
  ...
  plans.push(
    plan([{ type: 'update_tokens', projectId: second.id, tokens: { primary: '#ff0000' } }]),
  );
  const crossScope = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '修改其他项目',
  });
  ...
  assert.equal(
    (await api(`/agent/sessions/${session.id}/messages`, 'POST', { content: '创建另一个项目' }))
      .status,
    409,
  );
  assert.equal(
...
test('model planning cannot approve images and explicit image review gates reconstruction', async () => {
  ...
  assert.equal(
    (await api(`/agent/sessions/${session.id}/messages`, 'POST', { content: '模型自行批准' }))
      .status,
    502,
  );
  plans.push(plan([{ type: 'reconstruct_design' }]));
  assert.equal(
    (await api(`/agent/sessions/${session.id}/messages`, 'POST', { content: '跳过图片' })).status,
    409,
  );
  ...
  const wrong = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'approve_image',
      projectId: project.id,
      revision: generated.body.project.revision,
      imageUrl: '/api/assets/wrong.png',
    },
  });
  assert.equal(wrong.status, 409);
  const approved = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'approve_image',
      projectId: project.id,
      revision: generated.body.project.revision,
  ...
  assert.equal(approved.body.project.generation.approved, true);
  const reconstructed = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'reconstruct_design',
      projectId: project.id,
      revision: approved.body.project.revision,
      imageUrl,
    },
  });
...
test('concurrent project changes reject a stale planned mutation and preserve actual state', async () => {
  ...
  assert.equal(reply.body.message.actions[0].status, 'failed');
  const stored = (await api(`/projects/${project.id}`)).body;
  assert.equal(stored.name, 'Changed elsewhere');
  assert.equal(stored.tokens.primary, tokens.primary);
  assert.equal(reply.body.project.name, 'Changed elsewhere');
...
test('sync requires a reviewed session preview and rejects stale revisions or file changes', async () => {
  ...
  project = (
    await api('/workspace/bind', 'POST', { projectId: project.id, kind: 'local', path: workspace })
  ).body.project;
  const session = (await api('/agent/sessions', 'POST', { projectId: project.id })).body;
  const fabricated = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'apply_sync',
      projectId: project.id,
      revision: project.revision,
      previewId: 'fabricated',
    },
  });
  assert.equal(fabricated.status, 409);
  plans.push(plan([{ type: 'preview_sync' }]));
  const preview = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '同步设计到代码',
  });
  assert.equal(preview.status, 200);
  assert.equal(preview.body.syncPreview.files.length, 5);
  const stale = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'apply_sync',
      projectId: project.id,
      revision: project.revision - 1,
      previewId: preview.body.syncPreview.previewId,
    },
  });
  assert.equal(stale.status, 409);
  const applied = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'apply_sync',
      projectId: project.id,
      revision: project.revision,
  ...
  assert.match(
    await readFile(path.join(workspace, 'forma-generated', 'index.tsx'), 'utf8'),
    /FormaPage/,
  );
  const replay = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'apply_sync',
      projectId: project.id,
      revision: project.revision,
  ...
  plans.push(plan([{ type: 'preview_sync' }]));
  const second = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '再次预览',
  });
  await writeFile(path.join(workspace, 'forma-generated', 'tokens.css'), 'manual change');
  const changed = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    action: {
      type: 'apply_sync',
      projectId: project.id,
      revision: project.revision,
      previewId: second.body.syncPreview.previewId,
    },
  });
  assert.equal(changed.status, 409);
  assert.match(changed.body.error, /预览后/);
  assert.equal(
...
test('chat mutations respect established automatic sync without bypassing first manual synchronization', async () => {
  ...
  project = (
    await api(`/projects/${project.id}`, 'PUT', {
      ...project,
      workspace: { ...project.workspace, autoSync: true },
    })
  ).body;
  const session = (await api('/agent/sessions', 'POST', { projectId: project.id })).body;
  plans.push(plan([{ type: 'update_tokens', tokens: { primary: '#010203' } }]));
  const beforeFirstSync = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '更新主题',
  });
  ...
  project = (
    await api('/sync/apply', 'POST', {
      projectId: project.id,
      revision: beforeFirstSync.body.project.revision,
    })
  ).body.project;
  plans.push(plan([{ type: 'update_tokens', tokens: { primary: '#040506' } }]));
  const automatic = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '再次更新主题',
  });
  assert.equal(automatic.status, 200);
  assert.equal(automatic.body.message.actions[0].autoSynced, true);
  ...
  const conflicted = await api(`/agent/sessions/${session.id}/messages`, 'POST', {
    content: '保留设计修改但保护本地文件',
  });
  assert.equal(conflicted.status, 200);
  assert.equal(conflicted.body.project.tokens.primary, '#070809');
...
test('a session rejects a second turn while its model request is still running', async () => {
  ...
  try {
    ...
    assert.equal(
      (await api(`/agent/sessions/${session.id}`)).body.messages.at(-1).status,
      'pending',
    );
  } finally {
    release();
