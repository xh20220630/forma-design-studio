...
import { designContextHash } from './design-context.mjs';
import { localAssetFilename, validateSafeSvg } from './assets.mjs';
...
export function validateId(id) { requireValue(typeof id === 'string' && idPattern.test(id), 'ID 仅允许字母、数字、短横线和下划线，长度 1–100。'); return id; }
export function validColor(value) { return typeof value === 'string' && colorPattern.test(value); }
function finite(value, min = -100000, max = 100000) { return typeof value === 'number' && Number.isFinite(value) && value >= min && value <= max; }
function record(value) { return value && typeof value === 'object' && !Array.isArray(value); }
function text(value, max = 500) { return typeof value === 'string' && value.length <= max; }
function choices(value, values, message) { if (value !== undefined) requireValue(values.includes(value), message); }
...
function validateOptionalNodeFields(node) {
  for (const key of ['visible', 'locked', 'flipX', 'flipY', 'closed', 'clipContent', 'aspectRatioLocked']) if (node[key] !== undefined) requireValue(typeof node[key] === 'boolean', `节点 ${key} 必须是布尔值。`);
  for (const key of ['strokeWidth', 'blur', 'padding', 'paddingX', 'paddingY']) if (node[key] !== undefined) requireValue(finite(node[key], 0, 10000), `节点 ${key} 无效。`);
  for (const key of ['rotation', 'letterSpacing']) if (node[key] !== undefined) requireValue(finite(node[key]), `节点 ${key} 无效。`);
  ...
  for (const key of ['sizingHorizontal', 'sizingVertical']) choices(node[key], ['fixed', 'hug', 'fill'], '尺寸模式无效。');
...
export function validateTokens(tokens) {
  ...
  for (const key of ['primary', 'background', 'surface', 'text', 'muted', 'border']) requireValue(validColor(tokens[key]), `无效的颜色 token：${key}`);
  ...
  return Object.fromEntries(tokenKeys.map(key => [key, tokens[key]]));
}

export function validateNodes(nodes, components = [], scope = '页面') {
  ...
  for (const node of nodes) {
    ...
    for (const key of ['x', 'y', 'width', 'height']) requireValue(finite(node[key], key === 'width' || key === 'height' ? 0 : -100000), `节点 ${node.id} 的 ${key} 无效。`);
    for (const key of ['fill', 'color', 'stroke']) if (node[key] !== undefined) requireValue(validColor(node[key]), `节点 ${node.id} 的 ${key} 无效。`);
    if (node.text !== undefined) requireValue(typeof node.text === 'string' && node.text.length <= 50000, '节点文本过长。');
    for (const key of ['fontSize', 'radius', 'gap']) if (node[key] !== undefined) requireValue(finite(node[key], 0, 10000), `节点 ${key} 无效。`);
...
export function validateProject(project, { historicalSnapshot = false } = {}) {
  ...
  const ids = new Set();
  for (const entry of [...project.pages, ...project.components]) {
    ...
    if (entry.grid !== undefined) {
      ...
      for (const key of ['gutter', 'margin']) if (grid[key] !== undefined) requireValue(finite(grid[key], 0, 10000), '网格间距无效。');
    }
...
export function requireApproved(project) {
...
export function requireCurrentImage(project) {
