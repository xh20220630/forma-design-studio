...
test('provider errors and real conversation history persist across a new process', async () => {
  ...
  const child = await promisify(execFile)(process.execPath, ['--input-type=module', '-e', "import {getAgentSession} from './server/agent.mjs'; process.stdout.write(JSON.stringify(await getAgentSession(process.argv[1])));", session.id], { cwd: process.cwd(), env: process.env });
