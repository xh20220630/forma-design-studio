...
import { booleanNodes, type BooleanOperation } from "../lib/editor-boolean";
import {
  applyAutoLayout,
  boundsOf,
  cloneNodes,
...
export default function DesignEditor({
  ...
  const selection =
      page?.nodes.filter((node) => selectedIds.includes(node.id)) ?? [],
  ...
  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      ...
      if (active.type === "marquee") {
        ...
        setMarquee(box);
        const hits = active.nodes
          .filter(
            (n) =>
              !lockedNode(n, active.nodes) &&
              visibleNode(n, active.nodes) &&
              n.x < box.x + box.width &&
      ...
      if (active.type === "move") {
        ...
        if (snap && !event.altKey) {
          const b = boundsOf(
              active.nodes.filter((n) => active.ids.includes(n.id)),
            ),
            targets = active.nodes.filter(
              (n) => !active.ids.includes(n.id) && visibleNode(n, active.nodes),
            ),
            xs = [
              0,
              page.width / 2,
        ...
        const point = worldPoint(event.clientX, event.clientY),
          origin = active.newNode,
          nextPoint = { x: point.x - origin.x, y: point.y - origin.y },
          last = origin.points![origin.points!.length - 1];
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (mod && key === "z") {
        ...
        setSelectedIds(
          page.nodes
            .filter(
              (n) => !lockedNode(n, page.nodes) && visibleNode(n, page.nodes),
            )
            .map((n) => n.id),
        );
  ...
  const visibleLayers = (parentId?: string, depth = 0): ReactNode => {
    if (depth > 30) return null;
    return [...page.nodes]
      .reverse()
      .filter((n) => (n.parentId ?? undefined) === parentId)
      .map((node) => {
        const children = page.nodes.some((n) => n.parentId === node.id),
          Icon = nodeIcons[node.type] ?? Square,
          matches =
        ...
        return (
          <div key={node.id}>
            ...
            {!collapsed.has(node.id) && visibleLayers(node.id, depth + 1)}
          </div>
        );
      });
  };
                ...
                <div
                  ...
                  onDrop={(event) => {
                    if (event.target === event.currentTarget) {
                      const id = event.dataTransfer.getData("text/forma-node");
                      if (id) updateNode(id, { parentId: undefined });
                    }
                  }}
                >
                  {visibleLayers()}
                    ...
                    >
                      {page.nodes.map((node) => (
                        <div
                          ...
                          className={`ed-design-node ed-kind-${node.type} ${selectedIds.includes(node.id) ? "is-selected" : ""} ${node.locked ? "is-locked" : ""}`}
                          style={{
                            ...getNodeStyle(node, tokens, page.nodes, project),
                            pointerEvents:
                              tool === "select" || tool === "hand" || spaceDown
                                ? "auto"
                                : "none",
                          }}
                          ...
                          onDoubleClick={(event) => {
                            event.stopPropagation();
                            if (lockedNode(node, page.nodes)) return;
                            if (
                              node.type === "text" ||
                              node.type === "button"
                            ) {
                        ...
                        >
                          {editingText === node.id ? (
                            ...
                            <NodeContent node={node} project={project} />
                          )}
                          {node.prototype && (
                            <span
                              className="ed-prototype-indicator"
                      ...
                      <div
                        className="ed-grid-overlay"
                        style={
                          page.grid?.enabled && page.grid.type !== "columns"
                            ? {
                                backgroundImage:
                                  "linear-gradient(to right, #ef444425 1px, transparent 1px),linear-gradient(to bottom, #ef444425 1px, transparent 1px)",
                                backgroundSize: `${page.grid.size}px ${page.grid.size}px`,
                              }
                            : {}
                        }
                      >
                        {page.grid?.enabled && page.grid.type === "columns" && (
                          <div
                            className="ed-columns-grid"
                            style={{
                              padding: `0 ${page.grid.margin ?? 32}px`,
                              gap: page.grid.gutter ?? 20,
                            }}
                          >
                            {Array.from(
                              { length: page.grid.columns ?? 12 },
                              (_, i) => (
                                <span key={i} />
                              ),
                            )}
                      ...
                      {[...guides, ...smartGuides].map((guide, index) => (
                        <div
                          key={`${guide.axis}-${index}`}
                          className={`ed-guide ${guide.axis} ${index >= guides.length ? "smart" : ""}`}
                          style={
                            guide.axis === "x"
                              ? { left: guide.value, width: 1 / zoom }
                              : { top: guide.value, height: 1 / zoom }
                          }
                        />
                      ))}
                      {selection
