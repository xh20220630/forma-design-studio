...
export default function CanvasRulers({ zoom, origin, viewport, onFit, onGuide }: Props) {
  return (
    <div className="ed-viewport-rulers">
      <div className="ed-viewport-ruler is-horizontal" title="点击添加垂直参考线" onPointerDown={(event) => onGuide("x", event)}>
      ...
      <div className="ed-viewport-ruler is-vertical" title="点击添加水平参考线" onPointerDown={(event) => onGuide("y", event)}>
      ...
      <button type="button" className="ed-ruler-origin" title="适应画布 (Shift 1)" aria-label="适应画布" onClick={onFit}>
        <Maximize size={11} />
      </button>
    </div>
  );
