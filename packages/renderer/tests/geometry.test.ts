...
import { inverse, multiply, nodeMatrix, SpatialIndex, transform, transformedBounds } from '../src/canvas/geometry.ts';
...
test('spatial queries deduplicate spanning shapes and include large, negative and zero-height geometry', () => {
  const index = new SpatialIndex<{ id: string; bounds: { x: number; y: number; width: number; height: number } }>();
  ...
  assert.deepEqual(new Set(index.query({ x: -50, y: -10, width: 600, height: 20 }).map(item => item.id)), new Set(['page', 'line']));
...
test('ten thousand nodes return only the local visible candidates', () => {
  const index = new SpatialIndex<{ bounds: { x: number; y: number; width: number; height: number } }>();
  for (let y = 0; y < 100; y++) for (let x = 0; x < 100; x++) index.insert({ bounds: { x: x * 100, y: y * 100, width: 50, height: 50 } });
