...
test('serves the workbench document, assets, writes, and rejects foreign origins', async () => {
  ...
  try {
    ...
    const document = await documentResponse.json() as { revision: number; flows: Array<{ nodes: unknown[] }> };
    ...
    assert.deepEqual(((await viewportResponse.json()) as { viewports: Record<string, unknown> }).viewports.order, { x: 80, y: 64, zoom: 0.4 });

    const changeResponse = await fetch(`${server.url}/api/changes`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', origin: server.url },
      body: JSON.stringify({ baseRevision: 0, operations: [{ type: 'set-node-position', flowId: 'order', pageId: 'list', x: 120, y: 80 }] }),
    });
    assert.equal(changeResponse.status, 200);
    assert.equal(((await changeResponse.json()) as { revision: number }).revision, 1);

    const removedReviewResponse = await fetch(`${server.url}/api/changes`, {
      method: 'POST', headers: { 'content-type': 'application/json', origin: server.url },
      body: JSON.stringify({ baseRevision: 1, operations: [{ type: 'set-review', pageRef: 'order/list', review: { status: 'approved', notes: [] } }] }),
    });
    ...
    const foreignResponse = await fetch(`${server.url}/api/document`, { headers: { origin: 'https://malicious.example' } });
    ...
    const invalidChange = await fetch(`${server.url}/api/changes`, {
      method: 'POST', headers: { 'content-type': 'application/json', origin: server.url },
      body: JSON.stringify({ baseRevision: 1, operations: [{ type: 'set-node-position', flowId: 'order', pageId: 'list', x: 'bad', y: 1 }] }),
    });
