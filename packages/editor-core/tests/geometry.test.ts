...
test('horizontal and vertical auto-layout apply padding, gaps and cross-axis alignment', () => {
  const horizontal = [
    ...
    node('a', { parentId: 'frame', width: 40, height: 20 }),
    node('b', { parentId: 'frame', width: 60, height: 40 }),
    node('hidden', { parentId: 'frame', x: 999, visible: false }),
  ];
  ...
  const column = applyAutoLayout(
    [
      ...
      node('a', { parentId: 'frame', width: 60, height: 20 }),
      node('b', { parentId: 'frame', width: 80, height: 30 }),
    ],
    'frame',
  );
...
test('wrap layout moves overflowing children to rows using the previous row height', () => {
  const result = applyAutoLayout(
    [
      ...
      node('a', { parentId: 'frame', width: 70, height: 20 }),
      node('b', { parentId: 'frame', width: 60, height: 40 }),
      node('c', { parentId: 'frame', width: 30, height: 10 }),
    ],
    'frame',
  );
...
test('hug layout measures visible content and keeps padding around empty containers', () => {
  ...
  const result = applyAutoLayout(
    [
      frame,
      node('a', { parentId: 'frame', width: 50, height: 20 }),
      node('b', { parentId: 'frame', width: 80, height: 30 }),
      node('hidden', {
        parentId: 'frame',
        x: 1000,
...
test('fill shares the remaining main-axis space and independently fills the cross-axis', () => {
  const row = applyAutoLayout(
    [
      ...
      node('fixed', { parentId: 'frame', width: 100 }),
      node('fill-a', {
        parentId: 'frame',
        width: 20,
      ...
      node('fill-b', { parentId: 'frame', width: 20, sizingHorizontal: 'fill' }),
    ],
    'frame',
  );
  ...
  const column = applyAutoLayout(
    [
      ...
      node('fixed', { parentId: 'frame', height: 40 }),
      node('fill', {
        parentId: 'frame',
        width: 40,
        height: 20,
...
test('group resizing scales geometry while frame resizing honors child constraints', () => {
  ...
  assert.deepEqual(
    geometry(
      get(resizeNodes([container, child], 'container', { width: 200, height: 40 }), 'child'),
    ),
    [50, 30, 60, 10],
  );
  assert.deepEqual(
    geometry(
      get(
        resizeNodes([{ ...container, type: 'frame' }, child], 'container', {
          width: 200,
          height: 40,
        }),
        'child',
...
test('resizing vector paths scales local points and closed linear path coordinates', () => {
  ...
  const linear = resizeNodes(
    [{ ...path, points: undefined, path: 'M0 0 L40 0 L40 50 Z' }],
    'path',
    { width: 80, height: 25 },
  )[0];
  assert.deepEqual(
    linear.path.match(/[-+]?(?:\d*\.\d+|\d+\.?\d*)(?:[eE][-+]?\d+)?/g).map(Number),
    [0, 0, 80, 0, 80, 25],
  );
...
test('cloning a selection remaps its entire parent graph and isolates mutable overrides', () => {
  const nodes = [
    ...
    node('child', {
      ...
      parentId: 'parent',
      componentId: 'master',
      overrides: { label: { text: 'Original' } },
    }),
    node('leaf', { parentId: 'child' }),
    node('unrelated'),
  ];
